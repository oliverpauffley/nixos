...
* Config
** Initial Setup
first disable some unwanted UI elements
#+begin_src elisp
...
for mac we need some additional settings
#+begin_src elisp
(when (eq system-type 'darwin)
  (setq mac-option-modifier 'meta)
  (add-to-list 'exec-path "/etc/profiles/per-user/ollie/bin")
)
#+end_src
...
(defun my/org-capture-delete-frame ()
  "Delete frame if it was created for org-capture."
  (when (equal (frame-parameter nil 'name) "org-capture")
    (delete-frame)))

(add-hook 'org-capture-after-finalize-hook #'my/org-capture-delete-frame)

;; Make org-capture fill the entire frame
(add-hook 'org-capture-mode-hook #'delete-other-windows)

;; Make org-capture template selection fullscreen
(add-to-list 'display-buffer-alist
             '("\\*Org Select\\*" (display-buffer-full-frame)))
#+end_src

sort out backup files
#+begin_src elisp
...
(defun my-replace-or-yank ()
  ...
  (defun meow-setup ()
    ...
    (meow-normal-define-key
     ...
     '("X" . meow-goto-line)
     '("y" . meow-save)
     '("Y" . meow-sync-grab)[(uw.otel.v1.attribute) = { name: "basket_id" } ]
  ...
  (use-package meow
    ...
    ;; magit and ediff: use motion state so native keybindings pass through
    (add-to-list 'meow-mode-state-list '(magit-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-status-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-diff-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-log-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-process-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-revision-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-stash-mode . motion))
    (add-to-list 'meow-mode-state-list '(magit-refs-mode . motion))
    (add-to-list 'meow-mode-state-list '(forge-topic-mode . motion))
...
#+end_src
** Repeat-FU
getting vim like repetition of commands
#+begin_src elisp
...
#+begin_src elisp
;; Main typeface
(set-face-attribute 'default nil :family "Mononoki Nerd Font Propo" :height 200)

;; Proportionately spaced typeface
(set-face-attribute 'variable-pitch nil :family "DepartureMono Nerd Font" :height 0.8)

;; Monospaced typeface
(set-face-attribute 'fixed-pitch nil :family "Mononoki Nerd Font Propo" :height 1.0)
#+end_src
** Org
#+begin_src elisp
  ...
  (use-package org
    ...
    (setq org-default-notes-file "~/org/todo.org"
          ...
          '(("t" "Todo" entry (file+headline "~/org/todo.org" "Inbox")
             "* TODO %?\n%U\n" :empty-lines 1)
            ("b" "Brag List" entry (file+olp+datetree "~/org/work/performance_and_pay.org" "Achievements")
             "* %? %^g" :time-prompt t :tree-type week)
            ("j" "Journal Entry"
         ...
         :empty-lines 1)
          ...
          org-agenda-prefix-format  '((agenda . " %i %?-12t% s") (todo . " %i %-12:c") (tags . " %i %-12:c") (search . " %i %-12:c"))
          ...
          org-todo-keywords '((sequence "TODO(t)" "IN-PROGRESS(i)" "|" "DONE(d)" "CANCELED(c)"))
...
I want to start emacs with org agenda (month view) open
#+begin_src elisp
...
Summarize my work achievements brag list. Must be run from the file
#+begin_src elisp
(defun my-org-summarize-achievements (start-date end-date)
  "Summarize Org achievements by tag between START-DATE and END-DATE."
  (interactive
   (list
    (org-read-date nil nil nil "Start date: ")
    (org-read-date nil nil nil "End date: ")))
  
  (let ((tag-groups (make-hash-table :test 'equal)))
    
    (org-map-entries
     (lambda ()
       (when (= (org-outline-level) 5)
         (let* ((title (nth 4 (org-heading-components)))
                (tags (org-get-tags))
                (date-str (save-excursion
                            (when (org-up-heading-safe)
                              (let* ((parent-title (nth 4 (org-heading-components)))
                                     (parsed-time (parse-time-string parent-title))
           ...
           (when (and date-str
                      (not (string< date-str start-date))
                      (not (string< end-date date-str)))
             (let ((entry-data (list :date date-str :title title :body body)))
               (setq tags (or tags '("untagged")))
               (dolist (tag tags)
                 (puthash tag (cons entry-data (gethash tag tag-groups)) tag-groups)))))))
     nil 'file)
    
    (let ((buf (get-buffer-create "*Achievements Summary*")))
      (with-current-buffer buf
        (erase-buffer)
        (org-mode)
        (insert (format "#+TITLE: Achievements Summary (%s to %s)\n\n" start-date end-date))
        (maphash
         (lambda (tag entries)
           (insert (format "* %s\n" (upcase tag)))
           (dolist (entry (reverse entries))
             (insert (format "** [%s] %s\n" (plist-get entry :date) (plist-get entry :title)))
...
#+end_src
** Ox-slack
slack compatible export
#+begin_src elisp
(use-package ox-slack)
#+end_src
** Slack
Using slack from within emacs
...
(use-package slack
  ...
  :custom
  (slack-extra-subscribed-channels (mapcar 'intern (list "technology")))
  :config
  (slack-register-team
...
(use-package ob-sql
  :init
  (append '((sql t)) org-babel-load-languages))
#+end_src
** ob-http
run http requests from org files with org babel
#+begin_src elisp
(use-package ob-http
  :init
...
(use-package smartparens
  ...
  ;; Don't auto-delete pair delimiters on backspace
...
(use-package consult
  ...
  :bind (:map global-map ("C-c b" . #'consult-buffer))
  )
#+end_src
** Consult GH
#+begin_src elisp
(defun my/get-github-repo-name ()
  "Get the username/repo string from the Magit origin remote."
  (let ((url (magit-get "remote" "origin" "url")))
    (if (and url (string-match "github\\.com[:/]\\(.*?\\)\\(\\.git\\)?$" url))
        (let ((repo-name (match-string 1 url)))
          repo-name)
      (message "Not a GitHub repository or no origin remote."))))

(defun my/get-push-branch-no-remote ()
  "Get the push target branch name without the remote prefix."
  (let* ((full-push (magit-get-push-branch))  ;; e.g., "origin/feature/auth"
         (remote    (magit-get-push-remote))) ;; e.g., "origin"
    (when (and full-push remote)
      (let ((prefix (concat remote "/")))
        (if (string-prefix-p prefix full-push)
            ;; Strip the "origin/" prefix
            (let ((branch-only (substring full-push (length prefix))))
              branch-only)
          (message "could not strip remote name"))))))

(defun my/get-current-repo-run-list ()
  (interactive)
  (let* ((repo (my/get-github-repo-name))
        (branch (my/get-push-branch-no-remote))
        (initial-prompt (format "%s --branch %s" repo branch)))
    (if repo
        (consult-gh-run-list initial-prompt nil nil nil nil))))
...
(defvar-keymap my-smerge-keymap
  :doc "smerge conflict resolution"
  "n" #'smerge-next
  "p" #'smerge-prev
  "u" #'smerge-keep-upper
  "l" #'smerge-keep-lower
...
(define-key my-git-keymap (kbd "c") my-git-create-keymap)
(define-key my-git-keymap (kbd "m") my-smerge-keymap)
(define-key global-map (kbd "C-c g") my-git-keymap)
#+end_src
*** Git-Timemachine
#+begin_src elisp
...
(use-package magit
  ...
  ;; Full-frame magit-status, restore windows on quit
  (setq magit-display-buffer-function #'magit-display-buffer-fullframe-status-v1)
  (setq magit-bury-buffer-function #'magit-restore-window-configuration)
  ;; Word-level diff highlighting in hunks
...
#+end_src
*** Pr reviews
Review prs from within emacs!
#+begin_src elisp
(use-package pr-review
  :config
  (setq pr-review-search-predefined-queries
        '(("is:pr review-requested:@me state:open archived:false sort:updated-desc -author:dependabot[bot] -author:stepsecurity-app[bot]" . "Requests")
          ("is:pr author:@me state:open archived:false sort:updated-desc" . "Mine")
          ))
...
(use-package ediff
  :defer t
  :config
  ;; Single-frame EDiff (no popup control frame)
  (setq ediff-window-setup-function #'ediff-setup-windows-plain)
  ;; Side-by-side comparison
  (setq ediff-split-window-function #'split-window-horizontally)

  ;; Save and restore window config around EDiff sessions
  ...
  (add-hook 'ediff-suspend-hook #'my-ediff-restore-window-config)

  ;; j/k navigation in ediff control panel
...
#+end_src
** Log view
used to view logs nicely
#+begin_src elisp
(use-package logview)
#+end_src
** LSP
#+begin_src elisp
;; work emacs mode for .art files
(load-file "~/code/work/brossa/tools/emacs/art-mode.el")
...
(use-package lsp-mode
  ...
  (lsp-register-client
   (make-lsp-client
    :new-connection (lsp-stdio-connection "~/.local/bin/brossa-lsp-server")
  ...
  :bind (:map global-map
              ...
              ("C-c c d" . #'consult-lsp-diagnostics)
              )
  :hook
...
#+end_src
** Feature Mode
For cucumber style testing.
#+begin_src elisp
(use-package feature-mode
  :mode "\\.feature\\'"
  :interpreter "cucumber"
  :hook (feature-mode . my-feature-mode-setup)
  :config
  (setq feature-default-language "en")
  (setq feature-i18n-file nil)

  :init
  (defun my-feature-mode-setup ()
    (run-hooks 'prog-mode-hook)
    (setq tab-width 2)))
#+end_src
** Display Buffer Rules
We can use display buffer alist to tweak how windows are shown
#+begin_src elisp
(add-to-list 'display-buffer-alist
             '(
               ("\\*Meow Cheatsheet\\*"
...
#+end_src
** Rainbow mode
show rgb colours highlighted with their colour in buffers.
#+begin_src elisp
