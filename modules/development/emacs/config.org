...
** Decide-mode
Dice rolling and selecting from random tables
#+begin_src elisp
(defun my-decide-insert (what result)
  (let ((s (cond ((stringp decide-output-format)
    ...
    (when s (progn (kill-new s)
                   (minibuffer-message s)))))

(defun my-decide-roll (spec-string)
  "Roll some dice. insert into minibuffer and clipboard."
  (interactive
   (list
    (read-string "Roll: " "d")))
...
(use-package decide
  :bind (:map global-map ("C-c d" . #'my-decide-roll))
  )
#+end_src
